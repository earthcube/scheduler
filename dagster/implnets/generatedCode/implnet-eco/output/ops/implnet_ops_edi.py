...
def edi_gleaner(context):
    returned_value = gleanerio(("gleaner"), "edi")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def edi_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "edi")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def edi_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "edi")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def edi_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "edi")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def edi_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "edi")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def harvest_edi():
    harvest = edi_gleaner()
    #report1 =edi_missingreport_s3(harvest)
    report1 = edi_missingreport_s3(harvest)
    load1 = edi_nabu(harvest)
    load2 = edi_nabuprov(load1)
    load3 = edi_nabuorg(load2)
    load4 = edi_naburelease(load3)
