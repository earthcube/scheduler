...
def harvest_edi():
    ...
    load_uploadrelease = edi_uploadrelease(load_release)

    load_prune = edi_nabu_prune(load_uploadrelease)
    load_prov = edi_nabuprov(load_prune)
    load_org = edi_nabuorg(load_prov)

# run after load
    report_msgraph=edi_missingreport_graph(load_org)
    report_graph=edi_graph_reports(report_msgraph)
