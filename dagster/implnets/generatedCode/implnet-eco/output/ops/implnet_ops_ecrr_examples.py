...
def ecrr_examples_upload_summarize(context):
    returned_value = post_to_graph("ecrr_examples",path=SUMMARY_PATH, extension="nt", graphendpoint=_graphSummaryEndpoint())
    r = str('returned value:{}'.format(returned_value))
...
# run after load
    ...
    report_msgraph=ecrr_examples_missingreport_graph(start=load_org)
    report_graph=ecrr_examples_graph_reports(start=report_msgraph)
