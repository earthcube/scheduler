...
def designsafe_gleaner(context):
    returned_value = gleanerio(("gleaner"), "designsafe")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def designsafe_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "designsafe")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def designsafe_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "designsafe")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def designsafe_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "designsafe")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def designsafe_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "designsafe")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def harvest_designsafe():
    harvest = designsafe_gleaner()
    #report1 =designsafe_missingreport_s3(harvest)
    report1 = designsafe_missingreport_s3(harvest)
    load1 = designsafe_nabu(harvest)
    load2 = designsafe_nabuprov(load1)
    load3 = designsafe_nabuorg(load2)
    load4 = designsafe_naburelease(load3)
