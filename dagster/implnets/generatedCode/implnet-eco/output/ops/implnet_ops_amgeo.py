...
def harvest_amgeo():
    ...
    load_uploadrelease = amgeo_uploadrelease(load_release)

    load_prune = amgeo_nabu_prune(load_uploadrelease)
    load_prov = amgeo_nabuprov(load_prune)
    load_org = amgeo_nabuorg(load_prov)

# run after load
    report_msgraph=amgeo_missingreport_graph(load_org)
    report_graph=amgeo_graph_reports(report_msgraph)
