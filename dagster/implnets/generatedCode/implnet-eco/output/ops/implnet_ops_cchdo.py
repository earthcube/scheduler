...
def cchdo_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cchdo")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cchdo_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cchdo")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cchdo_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "cchdo")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cchdo_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "cchdo")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cchdo_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "cchdo")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
@graph
def harvest_cchdo():
    harvest = cchdo_gleaner()
    #report1 =cchdo_missingreport_s3(harvest)
    report1 = cchdo_missingreport_s3(harvest)
    load1 = cchdo_nabu(harvest)
    load2 = cchdo_nabuprov(load1)
    load3 = cchdo_nabuorg(load2)
    load4 = cchdo_naburelease(load3)
