...
def harvest_cchdo():
    ...
    load_uploadrelease = cchdo_uploadrelease(load_release)

    load_prune = cchdo_nabu_prune(load_uploadrelease)
    load_prov = cchdo_nabuprov(load_prune)
    load_org = cchdo_nabuorg(load_prov)

# run after load
    report_msgraph=cchdo_missingreport_graph(load_org)
    report_graph=cchdo_graph_reports(report_msgraph)
