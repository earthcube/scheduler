...
def cchdo_upload_summarize(context):
    returned_value = post_to_graph("cchdo",path=SUMMARY_PATH, extension="nt", graphendpoint=_graphSummaryEndpoint())
    r = str('returned value:{}'.format(returned_value))
...
# run after load
    ...
    report_msgraph=cchdo_missingreport_graph(start=load_org)
    report_graph=cchdo_graph_reports(start=report_msgraph)
