...
def neotomadb_gleaner(context):
    returned_value = gleanerio(("gleaner"), "neotomadb")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def neotomadb_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "neotomadb")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def neotomadb_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "neotomadb")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def neotomadb_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "neotomadb")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def neotomadb_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "neotomadb")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def harvest_neotomadb():
    harvest = neotomadb_gleaner()
    #report1 =neotomadb_missingreport_s3(harvest)
    report1 = neotomadb_missingreport_s3(harvest)
    load1 = neotomadb_nabu(harvest)
    load2 = neotomadb_nabuprov(load1)
    load3 = neotomadb_nabuorg(load2)
    load4 = neotomadb_naburelease(load3)
