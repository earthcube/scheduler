...
@op
def resourceregistry_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
@op(ins={"start": In(Nothing)})
def resourceregistry_nabu_prune(context):
    returned_value = gleanerio(context,("prune"), "resourceregistry")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def resourceregistry_nabuprov(context):
...
@op(ins={"start": In(Nothing)})
def resourceregistry_nabuorg(context):
...
@op(ins={"start": In(Nothing)})
def resourceregistry_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="resourceregistry")
    ...
    return
@op(ins={"start": In(Nothing)})
def resourceregistry_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="resourceregistry")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), MINIO_OPTIONS)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return
@op(ins={"start": In(Nothing)})
def resourceregistry_graph_reports(context) :
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="resourceregistry")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), MINIO_OPTIONS)
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def resourceregistry_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="resourceregistry")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), MINIO_OPTIONS)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "resourceregistry"
...
@op(ins={"start": In(Nothing)})
def resourceregistry_bucket_urls(context):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), MINIO_OPTIONS)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "resourceregistry"
...
@op(ins={"start": In(Nothing)})
def resourceregistry_summarize(context) :
    source_name = "resourceregistry"
...
def harvest_resourceregistry():
    containers = resourceregistry_getImage()
    harvest = resourceregistry_gleaner(start=containers)

# defingin nothing dependencies
    # https://docs.dagster.io/concepts/ops-jobs-graphs/graphs#defining-nothing-dependencies

    report_ms3 = resourceregistry_missingreport_s3(start=harvest)
    report_idstat = resourceregistry_identifier_stats(start=report_ms3)
    # for some reason, this causes a msg parameter missing
    report_bucketurl = resourceregistry_bucket_urls(start=report_idstat)

    #report1 = missingreport_s3(harvest, source="resourceregistry")
    load_release = resourceregistry_naburelease(start=harvest)
    load_uploadrelease = resourceregistry_uploadrelease(start=load_release)

    load_prune = resourceregistry_nabu_prune(start=load_uploadrelease)
    load_prov = resourceregistry_nabuprov(start=load_prune)
    load_org = resourceregistry_nabuorg(start=load_prov)

    summarize = resourceregistry_summarize(start=load_uploadrelease)

# run after load
    report_msgraph = resourceregistry_missingreport_graph(start=summarize)
    report_graph = resourceregistry_graph_reports(start=report_msgraph)
    report_msgraph=resourceregistry_missingreport_graph(start=load_org)
    report_graph=resourceregistry_graph_reports(start=report_msgraph)
