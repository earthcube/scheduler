...
def ucar_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ucar")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ucar_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ucar")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def ucar_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "ucar")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def ucar_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "ucar")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def ucar_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "ucar")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def harvest_ucar():
    ...
    #report1 =ucar_missingreport_s3(harvest)
    report1 = ucar_missingreport_s3(harvest)
    load1 = ucar_nabu(harvest)
    load2 = ucar_nabuprov(load1)
    load3 = ucar_nabuorg(load2)
    load4 = ucar_naburelease(load3)
