...
def unidata_gleaner(context):
    returned_value = gleanerio(("gleaner"), "unidata")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def unidata_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "unidata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def unidata_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "unidata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def unidata_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "unidata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def unidata_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "unidata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
@graph
def harvest_unidata():
    harvest = unidata_gleaner()
    #report1 =unidata_missingreport_s3(harvest)
    report1 = unidata_missingreport_s3(harvest)
    load1 = unidata_nabu(harvest)
    load2 = unidata_nabuprov(load1)
    load3 = unidata_nabuorg(load2)
    load4 = unidata_naburelease(load3)
