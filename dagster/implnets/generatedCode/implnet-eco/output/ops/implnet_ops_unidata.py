...
def harvest_unidata():
    ...
    load_uploadrelease = unidata_uploadrelease(load_release)

    load_prune = unidata_nabu_prune(load_uploadrelease)
    load_prov = unidata_nabuprov(load_prune)
    load_org = unidata_nabuorg(load_prov)

# run after load
    report_msgraph=unidata_missingreport_graph(load_org)
    report_graph=unidata_graph_reports(report_msgraph)
