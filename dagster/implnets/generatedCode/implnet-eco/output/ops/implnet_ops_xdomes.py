...
def xdomes_gleaner(context):
    returned_value = gleanerio(("gleaner"), "xdomes")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def xdomes_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "xdomes")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def xdomes_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "xdomes")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def xdomes_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "xdomes")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def xdomes_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "xdomes")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def harvest_xdomes():
    ...
    #report1 =xdomes_missingreport_s3(harvest)
    report1 = xdomes_missingreport_s3(harvest)
    load1 = xdomes_nabu(harvest)
    load2 = xdomes_nabuprov(load1)
    load3 = xdomes_nabuorg(load2)
    load4 = xdomes_naburelease(load3)
