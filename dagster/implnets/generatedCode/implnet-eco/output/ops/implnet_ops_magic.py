...
def harvest_magic():
    ...
    load_uploadrelease = magic_uploadrelease(load_release)

    load_prune = magic_nabu_prune(load_uploadrelease)
    load_prov = magic_nabuprov(load_prune)
    load_org = magic_nabuorg(load_prov)

# run after load
    report_msgraph=magic_missingreport_graph(load_org)
    report_graph=magic_graph_reports(report_msgraph)
