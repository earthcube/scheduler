...
def harvest_unavco():
    ...
    load_uploadrelease = unavco_uploadrelease(load_release)

    load_prune = unavco_nabu_prune(load_uploadrelease)
    load_prov = unavco_nabuprov(load_prune)
    load_org = unavco_nabuorg(load_prov)

# run after load
    report_msgraph=unavco_missingreport_graph(load_org)
    report_graph=unavco_graph_reports(report_msgraph)
