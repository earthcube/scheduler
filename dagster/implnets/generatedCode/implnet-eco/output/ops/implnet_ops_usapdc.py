...
def harvest_usapdc():
    ...
    load_uploadrelease = usapdc_uploadrelease(load_release)

    load_prune = usapdc_nabu_prune(load_uploadrelease)
    load_prov = usapdc_nabuprov(load_prune)
    load_org = usapdc_nabuorg(load_prov)

# run after load
    report_msgraph=usapdc_missingreport_graph(load_org)
    report_graph=usapdc_graph_reports(report_msgraph)
