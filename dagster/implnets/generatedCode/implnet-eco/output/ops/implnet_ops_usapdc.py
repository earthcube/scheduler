...
def usapdc_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usapdc")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usapdc_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usapdc")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def usapdc_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "usapdc")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def usapdc_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "usapdc")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def usapdc_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "usapdc")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
@graph
def harvest_usapdc():
    harvest = usapdc_gleaner()
    #report1 =usapdc_missingreport_s3(harvest)
    report1 = usapdc_missingreport_s3(harvest)
    load1 = usapdc_nabu(harvest)
    load2 = usapdc_nabuprov(load1)
    load3 = usapdc_nabuorg(load2)
    load4 = usapdc_naburelease(load3)
