...
def harvest_balto():
    ...
    load_uploadrelease = balto_uploadrelease(load_release)

    load_prune = balto_nabu_prune(load_uploadrelease)
    load_prov = balto_nabuprov(load_prune)
    load_org = balto_nabuorg(load_prov)

# run after load
    report_msgraph=balto_missingreport_graph(load_org)
    report_graph=balto_graph_reports(report_msgraph)
