...
def balto_gleaner(context):
    returned_value = gleanerio(("gleaner"), "balto")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def balto_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "balto")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def balto_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "balto")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def balto_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "balto")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def balto_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "balto")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
@graph
def harvest_balto():
    harvest = balto_gleaner()
    #report1 =balto_missingreport_s3(harvest)
    report1 = balto_missingreport_s3(harvest)
    load1 = balto_nabu(harvest)
    load2 = balto_nabuprov(load1)
    load3 = balto_nabuorg(load2)
    load4 = balto_naburelease(load3)
