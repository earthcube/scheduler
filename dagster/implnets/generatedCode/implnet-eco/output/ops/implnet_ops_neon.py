...
def harvest_neon():
    ...
    load_uploadrelease = neon_uploadrelease(load_release)

    load_prune = neon_nabu_prune(load_uploadrelease)
    load_prov = neon_nabuprov(load_prune)
    load_org = neon_nabuorg(load_prov)

# run after load
    report_msgraph=neon_missingreport_graph(load_org)
    report_graph=neon_graph_reports(report_msgraph)
