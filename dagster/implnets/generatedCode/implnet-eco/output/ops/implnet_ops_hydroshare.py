...
def hydroshare_gleaner(context):
    returned_value = gleanerio(("gleaner"), "hydroshare")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def hydroshare_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "hydroshare")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hydroshare_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "hydroshare")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hydroshare_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "hydroshare")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hydroshare_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "hydroshare")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def harvest_hydroshare():
    harvest = hydroshare_gleaner()
    #report1 =hydroshare_missingreport_s3(harvest)
    report1 = hydroshare_missingreport_s3(harvest)
    load1 = hydroshare_nabu(harvest)
    load2 = hydroshare_nabuprov(load1)
    load3 = hydroshare_nabuorg(load2)
    load4 = hydroshare_naburelease(load3)
