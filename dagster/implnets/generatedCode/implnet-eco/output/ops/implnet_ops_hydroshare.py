...
def hydroshare_upload_summarize(context):
    returned_value = post_to_graph("hydroshare",path=SUMMARY_PATH, extension="nt", graphendpoint=_graphSummaryEndpoint())
    r = str('returned value:{}'.format(returned_value))
...
# run after load
    ...
    report_msgraph=hydroshare_missingreport_graph(start=load_org)
    report_graph=hydroshare_graph_reports(start=report_msgraph)
