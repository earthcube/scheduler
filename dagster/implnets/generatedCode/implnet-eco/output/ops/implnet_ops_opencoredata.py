...
def opencoredata_gleaner(context):
    returned_value = gleanerio(("gleaner"), "opencoredata")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def opencoredata_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "opencoredata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def opencoredata_nabuprov(context, msg: str):
    returned_value = gleanerio(("prov"), "opencoredata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def opencoredata_nabuorg(context, msg: str):
    returned_value = gleanerio(("orgs"), "opencoredata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def opencoredata_naburelease(context, msg: str):
    returned_value = gleanerio(("release"), "opencoredata")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def harvest_opencoredata():
    harvest = opencoredata_gleaner()
    #report1 =opencoredata_missingreport_s3(harvest)
    report1 = opencoredata_missingreport_s3(harvest)
    load1 = opencoredata_nabu(harvest)
    load2 = opencoredata_nabuprov(load1)
    load3 = opencoredata_nabuorg(load2)
    load4 = opencoredata_naburelease(load3)
