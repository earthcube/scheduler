...
def aquadocs_upload_summarize(context):
    returned_value = post_to_graph("aquadocs",path=SUMMARY_PATH, extension="nt", graphendpoint=_graphSummaryEndpoint())
    r = str('returned value:{}'.format(returned_value))
...
# run after load
    ...
    report_msgraph=aquadocs_missingreport_graph(start=load_org)
    report_graph=aquadocs_graph_reports(start=report_msgraph)
