...
1. Check the VERSION file and make sure it has a value you want in it to be tagged to the containers.
1. ```make eco-clean```  will remove any existing generated code from the ./generatedCode/implnet-NETWORK directory
1. ```make eco-generate``` will build the code new.  Set the -d N in the makefile to a value N that is the number
   ...
   cron value you want.
1. ```make eco-build``` builds the Docker images following the build file ./build/Docker file.  Note this uses the 
...
1. ```make eco-push``` push to your container registry of choice, here docker.io
